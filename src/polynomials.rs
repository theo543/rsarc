...
pub fn newton_interpolation(y_values: &[GF64], x_values: &[GF64], poly: &mut [GF64], memory: &mut [GF64]) {
    assert_eq!(y_values.len(), x_values.len());
...
mod tests{
    ...
    fn can_roundtrip_interpolate_eval() {
        ...
        newton_interpolation(&y_vals, &x_vals, &mut poly, &mut memory);
    ...
    fn can_recover_lost_data() {
        ...
        newton_interpolation(&original_data, &original_x_values, &mut original_poly, &mut memory);
        ...
        newton_interpolation(&all_y_values[..50], &all_x_values[..50], &mut recovered_poly, &mut memory);
    ...
    fn insufficient_memory() {
        newton_interpolation(&[GF64(0); 10], &[GF64(0); 10], &mut [GF64(0); 10], &mut [GF64(0); 19]);
    }
}
